...
def main():
    ...
    # Conectar ao SQL Server usando Trusted Connection
    print(f"\n🔐 Conectando ao SQL Server usando Windows Auth...")
    
    try:
        # Usar Trusted Connection (Windows Authentication)
        ...
        print(f"✅ Conexão com SQL Server estabelecida")
        print(f"   Versão: {version[:60]}...")
        
        # Criar engine SQLAlchemy
        sql_engine = create_engine(f"mssql+pyodbc:///?odbc_connect={conn_str.replace(';', '%3B')}", fast_executemany=True)
        ...
        print(f"   - Sua conta {pyodbc.USER} tem permissão na base de dados {DATABASE}")
